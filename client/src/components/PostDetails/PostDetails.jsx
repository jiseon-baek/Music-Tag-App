...
const PostDetails = () => {
	...
	{recommendedPosts.length && (
		<div className={classes.section}>
			<Typography gutterBottom className={classes.recomTypo} variant="h5">I guess.. You might also like 🎵</Typography>
			<Divider />
			<div className={classes.recommendedPosts}>
				{recommendedPosts.map(({ title, message, name, likes, selectedFile, _id}) => (
					<div style={{ padding: '20px 40px 10px 40px', margin: '10px', cursor: 'pointer', border: '1px solid lightgrey', borderRadius: '20px', width: '200px'}} className={classes.recomPost} onClick={() => openPost(_id)} key={_id}>
						...
						<img src={selectedFile} width="200px" style={{ borderRadius: '12px', margin: '10px 0 0 0'}} alt="selectedFile"/>
					</div>
				))}
